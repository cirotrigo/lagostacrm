 ...
 * Cria um novo documento Q&A (pergunta/resposta) para treinamento
 */

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@/lib/supabase/server';
import { processDocument } from '@/lib/ai-training/processor';
import type { CreateQADocumentRequest } from '@/lib/ai-training/types';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Valida body
        const body = await request.json() as CreateQADocumentRequest;
        ...
        const { data: document, error: insertError } = await supabase
            ...
            .insert({
                ...
                type: 'qa',
                title: body.question.trim().slice(0, 100) + (body.question.length > 100 ? '...' : ''),
        ...
        console.error('Error in POST /api/ai-training/qa:', error);
        return NextResponse.json(
